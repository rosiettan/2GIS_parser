...
TABLE_COLUMNS = ['Название', 'Адрес', 'Вместимость', 'Ссылка']
...
def main():
    ...
    for _ in range(pages):
        ...
        for item in range(1, count_items + 1):
            ...
            quantity = get_element_text(driver, pathes.quantity)
            TABLE['Название'].append(title)
            TABLE['Адрес'].append(address)
            TABLE['Вместимость'].append(quantity)
    ...
    driver.quit()
    pd.DataFrame(TABLE).to_excel("file.xlsx")
